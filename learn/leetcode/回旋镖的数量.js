...
var numberOfBoomerangs = function(points) {
    ...
    for (let i = 0; i < n; i ++){
        ...
        let lineMap = new Map()
        for (let j = 0; j < n; j ++){
            tem[j] = j >= i ? Math.pow(points[i][0] - points[j][0], 2) + Math.pow(points[i][1] - points[j][1], 2) : map[j][i]
            record += 2 * (lineMap.get(tem[j]) || 0)
            lineMap.set(tem[j], (lineMap.get(tem[j]) || 0) + 1)
        }
        map.push(tem)
    }
    return record
